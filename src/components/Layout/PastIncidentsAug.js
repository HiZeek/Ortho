...
const PastIncidentsAug = (props) => {
  ...
  const [augDataList, setAugDataList] = useState([]);
  ...
  const incidentsNumber = augDataList.length;

  const dropdownHandler = () => {
    setDropdownIsActive((prevDropdownIsActive) => !prevDropdownIsActive);
  };

  const pastIncidentListItems = augDataList.map((item) => (
    <li
    ...
    >
      ...
      {item.button ? (
        <div className="font-bold tablet:text-lg phone:text-base">
          {/* <Link to={`/dataList/${augDataList.id}`}> */}
            <Button
    ...
    <Container>
      <div className="pt-14 pb-7">
        <div className="flex justify-between items-center border-b border-faint-black pb-4 mb-8">
          <p className="font-medium tablet:text-2xl phone:text-base">
            August 2022
          </p>
          <img
